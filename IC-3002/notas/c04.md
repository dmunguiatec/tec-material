...
\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwInput{Un número $k$ a buscar en una matriz $M$ de tamaño $n \times m$ que contiene números naturales ordenada ascendentemente de manera tal que $\forall i \in [1, n[, \forall j \in [1, m[, M_{i,j} \le M_{i,j+1} \land M_{i,j} \le M_{i+1, j}$}
