...
2. Considere la siguiente modificación del algoritmo de ordenamiento burbuja. ¿Esta modificación introduce casos al algoritmo? Justifique su respuesta.
...
\begin{algorithm}[H]
    ...
    \BlankLine
    \caption{Ordenamiento burbuja modificado}
    \SetAlgoVlined

    $i \assign 0$ \;
    $ordenado \assign no$ \;
    \While{$i < n \land \lnot ordenado$} {
        $i \assign i + 1$ \;
        $ordenado \assign si$ \;
        \For{$j \in [0 \twodots (n-i)[$} {
            \If{$A[j] > A[j+1]$} {
                $tmp \assign A[i]$ \;
                $A[i] \assign A[i+1]$ \;
                $A[i+1] \assign tmp$ \;
...
3. Analice el siguiente algoritmo para determinar si una hilera `s` de largo $m$ es subhilera de una hilera `S` de largo $n$ tal que $m \le n$. ¿Cuáles son los casos mejor, peor y promedio para la complejidad temporal de este algoritmo?

\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwInput{Una hilera $s$ de tamaño $m$ y una hilera $S$ de tamaño $n$ tal que $m \le n$}
    \KwOutput{$si$ cuando la hilera $s$ es una subhilera de $S$, $no$ en cualquier otro caso. Si $s = [s_1, s_2, \dots, s_m]$ y $s$ es subhilera de $S$ entonces $S = [\dots, s_1, s_2, \dots, s_m, \dots]$}
