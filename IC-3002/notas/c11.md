...
El abuso en la incorporación de candados en nuestro algoritmo puede afectar el rendimiento, generando largas colas de espera; y en casos extremos, puede terminar transformando nuestro algoritmo concurrente en uno secuencial.

Por otro lado, el uso de múltiples candados en un mismo hilo puede llevar al algoritmo a un punto muerto (*deadlock*) en el cada uno de los hilos termina bloqueado esperando a que algún otro hilo (también bloqueado) libere su candado. Recomendamos investigar "El problema de la cena de los filósofos" (*Dining philosophers* en la literatura en inglés) como un ejemplo clásico que ilustra la situación.

## Prueba de primalidad paralela ##

Consideremos ahora un ejemplo de algoritmo concurrente que involucre algunos de los elementos que hemos estudiado hasta el momento en la resolución del problema computacional de determinar si un número (grande) es primo o no.
