...
Para analizar quicksort primero debemos reconocer que los diferentes casos del algoritmo tienen cada uno su propia función $T(n)$ correspondiente. Consideremos cada caso entonces.

Recordamos que en el peor caso escogemos como pivote en cada subproblema el máximo o mínimo elemento de la lista, de forma que se produce una división desbalanceada del problema en dos subproblemas, uno de tamaño $n-1$ y otro de tamaño $1$.

La relación de recurrencia para el peor caso del quicksort es entonces

$$
T(n) = T(n-1) + T(1) + \mathcal{O}(n)
$$
